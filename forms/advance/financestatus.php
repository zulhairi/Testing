...
<body id="printTable">
  ...
  $sql = "SELECT * FROM advance_details WHERE aclaims_id = '".$id."'";
    ...
    <thead class="head">
      ...
      <thead class="head">
      <tr>
        <th scope="col" colspan="5" style="text-align: center; white-space: nowrap;">Approved Advance</th>
  ...
  $sql = "SELECT * FROM advance_details WHERE aclaims_id = '".$id."'";
          ...
          while ($row = mysqli_fetch_assoc($result)) 
          {
            ?>
            <tr class="table-light">
            <td scope="col" colspan = "5" style="text-align: center; white-space: nowrap; font-weight: bold;"><br>RM
            <input type="number" name = "approved" required style="width: 200px;" /></td>
          </tr>
        <?php
        }
        ?>
...
<div class = "container2">
  ...
  <table>
    ...
    <tr>
      <td class="cell-gap" valign="top">Mr. Yogan's Approval :</td>
...
$sql5 = mysqli_query($mysqli, "SELECT * FROM advanceclaim WHERE advance_id = '".$id."'");
  ...
  if (isset($_POST["submit"])) {
    $financeApproval = $_POST["finance_approval"];
    $remarks = $_POST["remark"];
    $approved = number_format((float)$_POST["approved"], 2, '.', '');
 
    if ($financeApproval == "1") {
      $finance_status = 'Approved';
    } else if ($financeApproval == "2") {
   ...
   $updateSql = "UPDATE approval_detail SET approval_status = '$finance_status',approval_remarks = '".$remarks."' WHERE staffno = '$ses_staffno' AND  t_claims_id = $id AND approval_priority = $next";

   if ($finance_status == "Declined") {
    ...
    $updateSql2 = "UPDATE advance_details SET advance_required = $approved WHERE aclaims_id = $id";

    $result2 = mysqli_query($mysqli, $updateSql2);
if (!$result2) {
       ...
       if (mysqli_query($mysqli, $updateSql)) {
           ...
           if($dql8["approval_priority"] == ""){
             ...
             if($ar1["imail"]!=""){
               ...
               Please Click <a href='https://intranet.minconsult.com/sources/E-FORM/?page=advace'>Here</a> To View And Verify
               <br /><br />
               Regards
               <br />
               E-Form System Services
...
?>
      ...
      <td class="cell-gap" valign="top">
        <select name="finance_approval" required>
